          ...
          <div className="grid grid-cols-2 gap-6 max-[900px]:grid-cols-1">
            {data.cards.map((card, i) => (
              <Reveal
                key={card.number}
                delay={i * 100}
                className="why-card bg-white border border-[#c8b8a2] rounded-[6px] p-8 hover:shadow-[0_4px_28px_rgba(58,46,40,0.08)] hover:-translate-y-1"
              >
                <p className="font-playfair text-[13px] italic font-semibold text-[#b8924a] mb-3">{card.number}</p>
                <h3 className="text-base font-semibold text-[#3a2e28] mb-3">{card.title}</h3>
                <p className="text-[14px] font-normal text-[#7b6b5a] leading-[1.8]">{card.description}</p>
              </Reveal>
            ))}
          </div>
        )}
