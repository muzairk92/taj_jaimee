  ...
  return (
    <section style={{ background: "var(--midnight)" }}>
      <Container className="py-20 max-[900px]:py-14">
        ...
        {data.heading && (
          <h2 className="font-playfair text-[32px] font-normal text-[#f0ebe0] text-center leading-[1.2] mb-10">
            {data.heading}
          </h2>
        )}
        ...
        {industries.length > 0 && (
          <div className="grid grid-cols-5 gap-4 max-[900px]:grid-cols-3 max-[640px]:grid-cols-2">
            {industries.map((industry) => (
              <div
                key={industry.label}
                className="rounded-[6px] px-4 py-6 text-center hover:bg-[rgba(240,235,224,0.09)] transition-colors"
                style={{
