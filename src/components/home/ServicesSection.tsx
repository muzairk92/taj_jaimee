  ...
  return (
    <section id="services" style={{ background: "var(--linen)" }}>
      <Container className="py-20 max-[900px]:py-14">
        ...
        {data.description && (
          <p className="text-[15px] font-medium text-[#7b6b5a] text-center leading-[1.8] max-w-[600px] mx-auto mb-12">
            {data.description}
          </p>
        )}
        ...
        {data.pillars && data.pillars.length > 0 && (
          <div className="grid grid-cols-3 gap-6 max-[900px]:grid-cols-1">
            {data.pillars.map((pillar) => (
              <div
                key={pillar.label}
                className="bg-white border border-[#c8b8a2] rounded-[6px] p-8 flex flex-col gap-4 hover:shadow-[0_4px_28px_rgba(58,46,40,0.08)] transition-shadow"
              >
                ...
                <p className="text-sm font-me text-[#7b6b5a] leading-[1.8] flex-1">{pillar.description}</p>
                {pillar.linkText && pillar.linkUrl && (
                  <a
                    href={pillar.linkUrl}
                    className="text-xs font-semibold tracking-[0.1em] uppercase text-[#b8924a] inline-block mt-auto hover:text-[#3a2e28] transition-colors"
                  >
                    {pillar.linkText}
                  </a>
                )}
