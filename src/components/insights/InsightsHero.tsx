...
export default function InsightsHero({ data }: { data: InsightsHeroData | null }) {
  if (!data) return null;
    ...
    <section className="relative overflow-hidden text-center" style={{ background: "var(--midnight)" }}>
      ...
      <Container className="py-24 max-[900px]:py-16 relative">
        <Reveal className="max-w-[780px] mx-auto">
          ...
          {data.description && (
            <p className="text-[14px] font-normal text-[rgba(240,235,224,0.7)] leading-[1.85] max-w-[680px] mx-auto mt-6">
              {data.description}
            </p>
          )}
        </Reveal>
